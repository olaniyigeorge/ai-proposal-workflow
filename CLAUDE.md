...
- **n8n → FastAPI intake is authenticated via a static shared-secret header** (env var on both sides, never inlined in the n8n workflow JSON) and is idempotent: dedupe on `hash(company_name + normalize(project_scope) + respondent_email)` before creating a `Proposal` (upsert, no-op on retry) — revised from an earlier `hash(timestamp + email_address)` scheme that only caught n8n retries, not a genuine resubmission. See `docs/decisions.md` #3–#4 and `docs/intake-schema.md`.
...
- **`salesperson_name` is optional and never a reliable `User` foreign key** (sales/admin *or the client* may submit the form; a client filling it directly leaves it blank). A blank value is normalized to `NULL`/unassigned, not stored as an empty string. Never auto-assign proposal ownership by string-matching this field — assignment is a manual claim/assign step in the review queue (resolved, see `docs/decisions.md` #20).
...
- **Delivery is unreachable except from `APPROVED`/`DOCUMENT_READY`**, is email-only with the PDF uploaded to storage and a **link** embedded in the email body (not an attachment — see `docs/reference/client-email-template.md`), and delivery status (sent/bounced/failed) must be tracked in `DeliveryRecord`. The composed email draft (with the link) must be reviewed by the salesperson before sending — no auto-send from `DOCUMENT_READY`.
...
- Delivery is email only, with the branded PDF uploaded to storage and a link embedded in the email body (not an attachment); the salesperson reviews the composed draft before it sends. Delivery status (sent/bounced/failed) must be recorded, not just fired-and-forgotten.

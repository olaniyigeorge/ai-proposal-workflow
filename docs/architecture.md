...
4. **A canonical facts/glossary layer for anything that must never vary.** Per `docs/intake-schema.md`, most template placeholders (client name, dates, pricing, timeline, scope) map directly to an intake field and must be reproduced verbatim, not left to the model to paraphrase — the strongest lever against numeric/factual drift across regenerations. `recommended_approach`, `deliverables`, and (as of 2026-09-11, see `docs/edge-cases.md` "Pinned sections read as pasted-in fragments, not part of one document") the Timeline/Pricing lead-ins and the Next Steps closing are the genuinely generated content; the timeline dates and price figure themselves are still pinned verbatim and appended after the generated lead-in, never something the model is asked to restate.
...
**Phase 8 — Activity logging & audit trail — implemented 2026-09-10**
`ActivityLogEntry` (`backend/app/models/activity_log.py`) now covers every state-relevant action CLAUDE.md requires (created, edited, regenerated, section approved, proposal approved, changes requested, rejected, document generated/failed, delivered/failed) plus a `*_FAILED` variant for every async step, matching the state machine's own failure-substate pattern. Written via `record_activity()` (`services/activity_log_service.py`), which only calls `db.add()` and lets the caller's existing commit persist it atomically with the action it documents — never a separate commit that could leave an action logged without happening, or vice versa. Dashboard-viewable per proposal (`GET /proposals/{id}/activity`, newest-first) and exportable as CSV for compliance (`GET /activity/export`, optionally filtered by proposal, oldest-first) per decisions #19. `actor` (the salesperson's email) is threaded from the request through every background job so an async outcome (e.g. a regeneration that completes minutes later) still records who asked for it; `None` for the one system-originated event (intake creating a Proposal — no salesperson identity exists at that point, decisions #20). Frontend: `web-app/components/proposals/ActivityTimeline.tsx`.

**Not done in this pass:** retention/access-control policy for the PII this table now also holds (decisions #18) is still unresolved — this phase built the mechanism, not the policy. **Audit log integrity** (§1's risk) is unaddressed at the DB level: this is still a normal table with normal CRUD permissions, so nothing stops direct DB access from editing/deleting a row — the application layer exposes no update/delete path (insert via `record_activity`, read via list/export only), which is as far as this phase goes. See `docs/edge-cases.md`.

**Phase 9 — Hardening — partially done 2026-09-10**
- **Done:** the 3-regeneration cap and mandatory-instruction requirement were already enforced and tested (decisions #13, prior phases). **Monitoring/alerting on failed jobs**: every background job failure (generation, regeneration, document generation, delivery) now logs a greppable `BACKGROUND_JOB_FAILED job=<name> proposal=<id> ...` tag (mirroring the `INTAKE_SCHEMA_DRIFT`/`INTAKE_AUTH_FAILED` pattern already built for intake — `app/main.py`, `app/api/v1/endpoints/intake.py`), so a log-based monitor can alert on any failed async job, not just intake, without depending on n8n at all.
- **Not done:** load/error-path testing on every integration boundary (this pass added targeted tests for the new activity-log call sites and the job-failure tags, not a systematic load-test pass); a security review of the auth model and RLS policies specifically (Supabase Auth is wired per Phase 0, but RLS itself has not been audited — the backend connects with a service-role-equivalent key and enforces authorization in the FastAPI dependency layer, not via Postgres RLS, which is worth confirming is the intended model rather than an oversight); and a final confirm-each-mitigation pass over every risk listed in §1. Flagged explicitly rather than marked done — see `docs/edge-cases.md` for the open items this leaves.

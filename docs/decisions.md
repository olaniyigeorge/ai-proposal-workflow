...
**Genuinely still open**, in priority order: #5 (lock the exact canonical field list), #9's follow-up (post-approval revision handling — a default is assumed, confirm it), #21's follow-up (cross-salesperson approval permission — a default is assumed, confirm it), and the assignment criteria for #20's claim/assign step (manual for now). Everything else in this log is resolved.

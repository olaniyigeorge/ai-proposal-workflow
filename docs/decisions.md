...
| 5 🟠 | What's the canonical intake schema, and who keeps Form/Sheet/API in sync? | Three places the same shape must match; nothing today catches drift when a form question is edited. | **Mechanism decided:** an n8n Code node normalizes Google Sheets field names to canonical API keys before the HTTP call (see `docs/intake-schema.md`). FastAPI's Pydantic model remains the enforced contract, 422s on drift. **Drift handling planned (2026-09-10, see `docs/edge-cases.md` "No error path when the Google Form drifts out of sync with the canonical intake schema"):** a required-field rename/removal is caught by FastAPI's 422 but currently has no n8n error branch to surface it — plan is an IF node on non-2xx routing to a dead-letter Sheet tab or alert. A new field silently added, or a question reworded without changing its key/column, can't be caught by any schema check — those are process gaps (update `docs/intake-schema.md` → n8n mapping → confirm against Pydantic), not code fixes. **Still open:** the exact canonical field list isn't fully locked — `docs/intake-schema.md` is the working draft; confirm it before Phase 1 schema freeze, and implement the n8n error branch directly in the n8n workflow (not version-controlled in this repo). | 2026-09-08 (drift plan added 2026-09-10) |

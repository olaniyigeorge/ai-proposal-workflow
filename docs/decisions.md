...
| 13b 🟠 | Does the regeneration endpoint preserve the salesperson's instruction trail across attempts? | Without it, the 3-attempt cap (#13) becomes frustrating: the salesperson re-types instructions from memory each attempt, repeats themselves, and can't tell whether the last regeneration actually addressed what they meant. | **Proposed for Phase 4:** the regeneration endpoint records each call's instruction (+ optional short note) in a per-section suggestion log, surfaced in the UI as attempt history ("attempt 1: 'make more formal' → attempt 2: 'shorten, keep pricing facts' → attempt 3: 'lead with the ROI line'"). The instruction is already mandatory per #13; the log is the visibility layer so attempts are informed rather than amnesiac. Storage: JSON list on ProposalSection or a separate regenerate-history table — decide in Phase 4. See `docs/edge-cases.md` "Regeneration loses the salesperson's instruction context across attempts". | 2026-09-09 |
...
**Genuinely still open**, in priority order: #5 (lock the exact canonical field list), #9's follow-up (post-approval revision handling — a default is assumed, confirm it), #21's follow-up (cross-salesperson approval permission — a default is assumed, confirm it), #13b (regeneration suggestion log — proposed for Phase 4, not yet decided), and the assignment criteria for #20's claim/assign step (manual for now). Everything else in this log is resolved.

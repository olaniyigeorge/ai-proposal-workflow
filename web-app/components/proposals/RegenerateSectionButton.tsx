...
export function RegenerateSectionButton({
    ...
    return (
      <div className="space-y-2 p-3 rounded-lg border border-violet-200 bg-violet-50">
        ...
        <textarea
          ...
          className="w-full text-sm text-[#1f2429] bg-white p-2.5 rounded-lg border border-violet-300 focus:border-violet-500 focus:outline-none disabled:opacity-60"
        />
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={closePanel}
            disabled={isSubmitting}
            className="px-3 py-1.5 rounded bg-white hover:bg-[#f5f6f5] border border-[#d8dbd9] text-[#1f2429] text-[11px] font-medium disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
          >
            Cancel
          </button>
        </div>
        {section.regeneration_log.length > 0 && (
          <div className="pt-2 border-t border-violet-200 space-y-1">
            <p className="text-[11px] text-[#5c646c] font-medium">Previous attempts</p>
            {section.regeneration_log.map((entry, i) => (
              <p key={i} className="text-[11px] text-[#5c646c] leading-relaxed">
                <span
                  className={
                    entry.outcome === 'succeeded' ? 'text-emerald-700' : 'text-rose-700'
                  }
                >
                  {i + 1}. {entry.outcome}
                </span>
  ...
  return (
    <button
      ...
      className="px-2.5 py-1 rounded bg-violet-50 border border-violet-200 text-violet-700 text-[11px] font-medium hover:bg-violet-100 disabled:bg-[#f5f6f5] disabled:border-[#d8dbd9] disabled:text-[#9aa0a6] disabled:cursor-not-allowed transition-colors"
    >
